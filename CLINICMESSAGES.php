...
if(@$_COOKIE['SR']==null){
	...
	$ID = $_SESSION["ID"];
	$verifyFirst = explode(" ",$_COOKIE['SR']);
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$sql = "INSERT INTO `messages`(`FromName`, `SenderID`, `ToName`, `ReceiverID`, `Message`, `Date`, `Time`,`DateTimeSort`, `MessageID`, `Seen`) VALUES 
		...
		if ($conn->query($sql) === TRUE) {
			echo "<script>window.location='CLINICMESSAGES.php';</script>";
		} else {
...
<link rel="stylesheet" href="CLINICMESSAGES.css" media="screen">
					...
					<option selected disabled></option>
						<?php
							...
							$result = $conn->query($sql);
								if ($result->num_rows > 0) {
									while($row = $result->fetch_assoc()) {
										echo '<option value="'.$row['PetOwnerID'].'">'.$row['FullName'].'</option>';
									}
								}
						?>
					</select>
						...
						$result = $conn->query($sql);
							$ID = $_SESSION["ID"];
								if ($result->num_rows > 0) {
									...
									$sql = "SELECT * FROM `petowner` where PetOwnerID='".$ReceiverID."'";
									$result = $conn->query($sql);
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$name = $row['FullName'];
				  ...
				  ?>
						...
						?>
							...
							<div class="u-text" style="overflow:hidden;width:6%;height:100%;float:right;">
								<button style="height:100%;width:100%;"><i class="bi bi-send"></i><br></button>
						 </div></form>
						 </div>
					</div>
                  </div>
